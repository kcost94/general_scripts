...
bowtie2 -p 4 --local --very-sensitive-local --phred33 -I 1 -X 500 -x $genome_dir/bowtie2-indexes/$genome -1 *_val_1.fq.gz -2 *_val_2.fq.gz -S $line\.sam > align-out.txt 2>&1
...
samtools sort -@ 8 -n -T $line.sorted -o $line-name-sorted.bam $line-sorted.bam
...
bedtools genomecov -bg -i filtered-$line\.bed -g $genome_dir/$genome.chrom.sizes > filtered-$line\.bg
bedGraphToBigWig filtered-$line\.bg $genome_dir/$genome.chrom.sizes filtered-$line\.bw

bedtools genomecov -bg -i $line\.bed -g $genome_dir/$genome.chrom.sizes > $line\.bg
bedGraphToBigWig $line\.bg $genome_dir/$genome.chrom.sizes $line\.bw
